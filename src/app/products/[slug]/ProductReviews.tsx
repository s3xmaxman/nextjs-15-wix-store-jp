...
export default function ProductReviews({ product }: ProductReviewsProps) {
  /**
   * useInfiniteQueryを使用して、商品レビューを無限スクロールで取得します。
   *
   * @param {Object} options - useInfiniteQueryのオプション
   * @param {Array} options.queryKey - クエリーキー。商品IDを含む。
   * @param {Function} options.queryFn - 非同期関数で、レビューデータを取得します。
   * @param {Function} options.select - 取得したデータをフィルタリングして返します。
   * @param {string|null} options.initialPageParam - 初期ページパラメータ。
   * @param {Function} options.getNextPageParam - 次のページのカーソルを取得する関数。
   */
        ...
        <div className="ms-10 mt-2.5 space-y-1 border-t pt-2.5">
          <div className="flex items-center gap-2">
            ...
            <Image
              ...
              className="size-5"
            />
            <span className="font-bold">Bad Shop Team</span>
          </div>
